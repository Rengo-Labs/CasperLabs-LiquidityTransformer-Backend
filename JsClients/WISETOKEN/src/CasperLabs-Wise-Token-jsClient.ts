import {
  ...
  class WISETokenClient {
	...
	public async createStakeWithSessionCode(
	  ...
	  const _referrer = new CLKey(
		new CLAccountHash(Uint8Array.from(Buffer.from(referrer, "hex")))
	  );
	  const runtimeArgs = RuntimeArgs.fromMap({
		package_hash: utils.createRecipientAddress(_packageHash),
		entrypoint: CLValueBuilder.string(entrypointName),
	...
	public async balanceOf(account: string) {
	  try {
		const key = new CLKey(
		  new CLAccountHash(Uint8Array.from(Buffer.from(account, "hex")))
		);
	...
	public async balanceOfcontract(hash: string) {
	  try {
		const _key = new CLByteArray(Uint8Array.from(Buffer.from(hash, "hex")));
		const key = createRecipientAddress(_key);
		const keyBytes = CLValueParsers.toBytes(key).unwrap();
		const itemKey = Buffer.from(keyBytes).toString("base64");
		const result = await utils.contractDictionaryGetter(
		  this.nodeAddress,
	...
	public async balanceOfBlock(hash: string, stateRootHash: any) {
	  try {
		const _key = new CLByteArray(Uint8Array.from(Buffer.from(hash, "hex")));
		const key = createRecipientAddress(_key);
		const keyBytes = CLValueParsers.toBytes(key).unwrap();
	...
	public async allowance(owner: string, spender: RecipientType) {
	  try {
		const keyOwner = new CLKey(
	...
	public async createStakeWithCsprJsclient(
	  ...
	  const referrerbytearray = new CLByteArray(
		Uint8Array.from(Buffer.from(referrer, "hex"))
	  );
	...
	public async createStakeWithTokenJsclient(
	  ...
	  const tokenbytearray = new CLByteArray(
		Uint8Array.from(Buffer.from(token_address, "hex"))
	  );
	  const referrerbytearray = new CLByteArray(
		Uint8Array.from(Buffer.from(referrer, "hex"))
	  );
	...
	public async checkReferralsByIdJsclient(
	  ...
	  const referrerbytearray = new CLByteArray(
		Uint8Array.from(Buffer.from(_referrer, "hex"))
	  );
	  const runtimeArgs = RuntimeArgs.fromMap({
		referrer: CLValueBuilder.key(referrerbytearray),
	...
	public async checkMatureStakeJsclient(
	  ...
	  const stakerbytearray = new CLByteArray(
		Uint8Array.from(Buffer.from(staker, "hex"))
	  );
	  const runtimeArgs = RuntimeArgs.fromMap({
